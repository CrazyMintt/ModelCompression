...
# ── 7B. PTQ Static via FX Graph Mode ─────────────────────────────────────────
from torch.ao.quantization.quantize_fx import prepare_fx, convert_fx
from torch.ao.quantization import get_default_qconfig_mapping

CPU = torch.device('cpu')

print('Aplicando PTQ Static (FX Graph Mode) ...')

ptq_static = copy.deepcopy(baseline_model).cpu().eval()
qconfig_mapping = get_default_qconfig_mapping('fbgemm')
example_input = torch.randn(1, 3, IMG_SIZE, IMG_SIZE)

ptq_static = prepare_fx(ptq_static, qconfig_mapping, example_input)

# Calibração
cpu_val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False,
                            num_workers=NUM_WORKERS, pin_memory=False)
with torch.no_grad():
    for i, (imgs, _) in enumerate(cpu_val_loader):
        ptq_static(imgs)
        if i >= 15:
            break

ptq_static = convert_fx(ptq_static)

# Avaliação
cpu_test_loader = DataLoader(test_ds, batch_size=BATCH_SIZE, shuffle=False,
                             num_workers=NUM_WORKERS, pin_memory=False)
ptq_static.eval()
correct, total = 0, 0
with torch.no_grad():
    for imgs, lbls in cpu_test_loader:
        preds = ptq_static(imgs).argmax(1)
        correct += (preds == lbls).sum().item()
        total += lbls.size(0)
acc = correct / total

size_mb = model_size_mb(ptq_static)
latency = measure_latency(ptq_static, device=CPU)
tot, nz = count_nonzero_params(baseline_model)

label = 'PTQ Static (INT8)'
print_metrics(label, acc, size_mb, latency, tot, nz)
save_result(label, acc, size_mb, latency, tot, nz)

torch.save(ptq_static.state_dict(), 'ptq_static_int8.pth')
print('Salvo: ptq_static_int8.pth')
